    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth  // Cambia authorizeRequests por authorizeHttpRequests
                        .requestMatchers(HttpMethod.POST, //Estos endpoints no necesitan autenticación
                                "/api/auth/login",
                                "/api/auth/logout")
                        .permitAll()
                        ...
                        .anyRequest().authenticated()) //Solo no pide autenticación para los endpoints de arriba -> "/api/auth/login" , "/api/auth/logout"
                .sessionManagement(sess -> sess
                        .sessionCreationPolicy(SessionCreationPolicy.STATELESS)) //Si al momento de hacer una request a cualquier endpoint, a este le llegara una parte este lo bota y ya no puede ser utilizada la API
